...
class BaiduIndex:
    ...
    def _format_data(self, data):
        ...
        for i, cur_date in enumerate(date_list):
            try:
                all_index = data[self._all_kind[0]]['data'][i]
                pc_index = data[self._all_kind[1]]['data'][i]
                wise_index = data[self._all_kind[2]]['data'][i]
            except IndexError:
                all_index, pc_index, wise_index = '', '', ''
            formated_data = {
                'date': cur_date.strftime('%Y-%m-%d'),
                'keyword': json.loads(keyword.replace('\'', '"'))[0]['name'],
                'area':self.area,
                'kind':'百度搜索指数',
                'time_type': data['type'],
                'all_index': int(all_index) if all_index else 0,
                'pc_index': int(pc_index) if pc_index else 0,
                'wise_index': int(wise_index) if wise_index else 0
            }
            yield formated_data
